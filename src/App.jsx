...
// Smart Mobile Scroll Hint
const ScrollHint = () => (
  <motion.div 
    initial={{ opacity: 0 }}
    whileInView={{ opacity: [0, 1, 1, 0] }}
    viewport={{ once: true, margin: "-20%" }} 
    transition={{ duration: 3.5, times: [0, 0.1, 0.8, 1] }} 
...
// --- UPDATED SHOP PAGE FOR MOBILE GRID ---
const ShopPage = ({ setView, setSelectedProduct, products }) => {
  ...
  return (
    <div className="pt-32 px-4 md:px-6 pb-20 max-w-7xl mx-auto min-h-screen">
      <div className="flex items-center gap-4 mb-8 md:mb-12">
        ...
        <div>
          <h2 className="text-3xl md:text-4xl font-serif italic text-white">The Collection</h2>
          <p className="text-[8px] md:text-[10px] tracking-[0.3em] uppercase text-gray-500 mt-2">Alpha Drop 01 // {products.length} Items</p>
        </div>
      </div>
      
      {/* UPDATED GRID LAYOUT:
          - Mobile: grid-cols-2 (2 columns) with smaller gap (gap-3) to fit screen.
          - Desktop: grid-cols-3 (3 columns) with larger gap.
      */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-x-4 md:gap-y-8 pb-20">
        {products.map((item) => (
          <motion.div 
            key={item.id} 
          ...
          >
            <div className="aspect-[3/4] overflow-hidden bg-white/5 mb-3 relative rounded-sm border border-white/5 group-hover:border-white/20 transition-colors">
              ...
              {item.stock !== "Sold Out" ? (
                <div className="absolute top-2 left-2 bg-black/60 backdrop-blur-md px-1.5 py-0.5 text-[6px] md:text-[8px] tracking-[0.2em] uppercase text-white border border-white/10">{item.stock} Left</div>
              ) : (
                <div className="absolute inset-0 bg-black/60 flex items-center justify-center"><span className="text-white text-[8px] md:text-xs font-bold tracking-[0.2em] uppercase border border-white px-2 py-1">Sold Out</span></div>
              )}
            </div>
            <div className="flex flex-col md:flex-row justify-between items-start gap-1">
              <div className="w-full">
                <h3 className="text-xs md:text-lg font-serif italic text-white group-hover:text-red-400 transition-colors truncate">{item.name}</h3>
                <p className="text-[6px] md:text-[10px] tracking-widest text-gray-500 uppercase mt-0.5 hidden md:block">260GSM Heavyweight</p>
              </div>
              <span className="text-xs md:text-base text-white font-mono">${item.price}</span>
            </div>
          </motion.div>
        ))}
      </div>
